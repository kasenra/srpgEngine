...
# Change to Items
@export var chests: Dictionary[Vector2, String] = {}
...
func getChest(cell: Vector2) -> Vector2:
	for d in NEIGHBORS:
		if _wallMap.get_cell_source_id(cell + d) == 3 && _wallMap.get_cell_atlas_coords(cell+d) == Vector2i(0,0):
			return cell + d
	return Vector2(0,0)
func openChest(cell: Vector2) -> String:
	var c = getChest(cell)
...
func getMoveCost(cell: Vector2, unit: Unit, weapon : bool = false) -> float: ## Provided a cell and a unit on this map, determine that unit's movecost for that cell
	# Doors and Walls are unwalkable
	if _wallMap.get_cell_source_id(cell) in [1,2,3]:
		return 999
	if !weapon:
		return Codex.TERRAIN_MOVE[Vector2i(idToTerrain[get_cell_source_id(cell)], unit.stat.moveType)]
	else:
		return Codex.TERRAIN_MOVE[Vector2i(idToTerrain[get_cell_source_id(cell)], Codex.MOVETYPE.ATTACK)]
